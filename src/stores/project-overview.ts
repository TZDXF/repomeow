...
export const useProjectOverviewStore = defineStore("project-overview", () => {
  ...
  const byId = ref<Record<number, ProjectOverview>>({});
  ...
  function refresh(projectId: number): Promise<ProjectOverview> {
    ...
    const p = cmd<ProjectOverview>("get_project_overview", { projectId })
      .then((data) => {
        ...
        if (keys.length >= MAX_ENTRIES && !(String(projectId) in map)) delete map[Number(keys[0])];
        map[projectId] = data;
        byId.value = map;
        return data;
      })
      .catch(() => cached(projectId) ?? ({ hidden_items: [], custom_commands: [] }) as ProjectOverview)
      .finally(() => {
  ...
  function setHiddenLocal(projectId: number, kind: HiddenKind, targetKey: string, hidden: boolean) {
    const cur = byId.value[projectId];
    if (!cur) return;
    ...
    byId.value = { ...byId.value, [projectId]: { ...cur, hidden_items: items } };
  }
