...
/** 媒体预览公共逻辑(ProjectFiles 与 ResourceSkillPreview 共用):
 *  图片/视频不读文本内容,走 asset 协议(convertFileSrc)直显;
...
export function useImagePreview(
  ...
  const isVideo = computed(() => VIDEO_EXTS.has(selectedExt.value));
  ...
  /** 视频展示地址(asset 协议 URL,range 请求由协议层处理);非视频或不可解析时为空串 */
  const videoSrc = computed(() => {
    const path = selected.value;
    if (!path || !isVideo.value) return "";
    const abs = toAbsolutePath(path);
    return abs ? convertFileSrc(abs) : "";
  });
