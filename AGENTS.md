...
- **wiki 自动增量更新(「跟踪更新」联动,默认关闭)**:设置页「跟踪更新」有开关与阈值(`stores/settings.ts` 的 `wikiAutoUpdate`/`wikiAutoUpdateThreshold`,默认 10);auto-pull 快进成功后 Rust emit `git://auto-pulled`(project_id + 本轮拉取提交数),主窗口 App.vue 据此调 `stores/wiki.ts` 的 `autoUpdate`(内部串行队列):「meta.headSha..HEAD 提交数 ≥ 阈值」才执行,复用手动增量更新的 `applyUpdate` 同一管线;开关关闭/正忙/无 wiki/无 headSha/历史改写时静默跳过,失败 toast 提示,**绝不自动整本重生成**(只留给用户手动触发)。
