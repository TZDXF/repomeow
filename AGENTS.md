...
- stale 检测:meta.headSha 与当前 HEAD(git2 `open_repo`,wiki.rs 内复用)不一致即过时;stale 时可**增量更新**(`wiki_changed_files` 取 headSha..HEAD 变更文件清单,仅重生成 relevantFiles 命中的页面并推进 meta.headSha;无 headSha/历史改写/**生成后端切换(meta.generator 不一致)**时退化为整本重生成)。
