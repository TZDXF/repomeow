...
11. **全局 AI 资源库**(`commands/ai/resource_library/` + `components/settings/Resource*`):设置页「资源管理」管理全局 Skills/MCP、Agent 部署目标显隐与备份,作为项目 AI 资产页 Skills/MCP 的唯一添加来源(项目页只选择和部署,不自定义内容)。数据固定在 `~/.repomeow/resource-library/`:明文 `library.json`、`skills.json`、`skills/<directory>/SKILL.md`,以及可选加密的 `mcp.json`;同步运行状态单独放仓库外 `~/.repomeow/resource-library-state.json`,避免同步后 Git 工作树变脏。Skill 支持多分组(分组带可选描述),正文 frontmatter 是 name/description 事实源;分组与技能是多对多关联、无独立全量重排命令,分组成员经 `rl_skill_group_set_skills` 以「该分组完整成员集合」整体差量设定。Skills Tab 卡片点击进入独立预览页(路由 `resource-skill`,`views/ResourceSkillPreview.vue`):token 统计(`rl_skill_tokens`,o200k 口径,返回描述+全部文件的内容指纹)、md 翻译(经 `ai_translate_markdown`,译文按「界面语言 + 内容 hash」缓存 IndexedDB 30 天)与**安全扫描**(`rl_skill_scan`,参考 SkillSpector 两层管线:静态正则规则层在库锁内执行;可选内置 Agent 语义层在锁外运行,提示词固定 `ai/prompts/skill-scan.md` 含反越狱框架、只读工具预算 20,模型缺省设置页 defaultModel、可显式指定;AI 未配置/失败/取消时静态结果照常返回并以 llm_status 标注,评分按严重度权重封顶 100、目录含可执行脚本 ×1.3)。扫描报告按技能内容指纹缓存进 IndexedDB(`lib/scan-cache.ts`,30 天),内容不变不重扫。MCP 支持粘贴 JSON 批量导入(`rl_mcp_import`:前端解析 mcpServers/servers/单定义/裸键值表四种结构,重名或校验失败的条目跳过并记 skipped,部分成功语义)。每次应用内修改自动本地 git 快照,配置 origin 后后台 fetch/fast-forward/push;分叉只允许用户选择远端 reset 或带显式租约的 `force-with-lease`,首次强制导入前保留带时间戳的完整备份目录。MCP 可由用户口令以 Argon2id + XChaCha20Poly1305 加密,口令/派生密钥只驻留进程内;启用加密会重建 Git 历史清除可达的明文 MCP 提交。前端所有 IPC 与后端字段映射集中在 `lib/resource-library.ts`,勿在组件散写命令名。
...
- 详情页内容区经分段控件切换「执行 / AI」两视图(`repomeow.project-detail-view` localStorage 全局记忆)。AI 页(`AiAssetsView.vue`)分为 Skills 分组树、MCP 列表、指令/规则文件;已移除 Agent 安装状态汇总、自定义 Skill/MCP 表单及项目 cc-switch 导入入口。`scan_project_ai_assets` 只读固定路径探测,Skills 按真实目录保留同名实例,不再按名称去重或探测 Agent 安装状态。
- **资源库部署**(`assets/deployment.rs` + `deployment_io.rs` + `deployment_mcp.rs`,前端桥 `lib/project-ai-resources.ts`):`project_ai_targets` 返回已适配的项目配置目标(claude/cursor/copilot/gemini/codex/opencode/zcode),Skills/MCP 区域分别通过 Agent 按钮打开选择面板。`project_ai_resources` 返回资源库候选、分组、部署状态与 revision,不返回 MCP 密钥;`project_ai_apply` 只接受项目路径、种类、Agent、资源 ID 完整选择集和 expectedRevision,不得恢复前端传任意定义/目标路径的旧写接口。源数据在库锁内读取,字段转换全部由 Rust 完成。Skills 分组只用于当前成员的批量选择,同一资源多分组展示、ID 选择联动去重,不是动态订阅;内容更新手动应用,不自动同步。
- **部署记录与安全**:记录位于 `~/.repomeow/project-ai/<归一化 canonical 项目路径的 git blob hash>.json`(version=1,当前 0.2.0 开发中),保存来源 ID、Agent、实际路径/名称、内容指纹,不保存 MCP 明文定义或密钥;来源改名不擅自更换已部署路径/服务器键。记录 revision 防止旧面板覆盖其他面板新配置。只移除当前项目/目标的托管项,非托管同名或本地修改一律拒绝覆盖/删除,共享物理路径仍有托管引用时只解除关联。来源删除或资源库上锁不触发卸载,来源不可用时仍可显式移除已有配置。批量操作逐项回报失败,先移除再添加;新增/更新先持久化意图,移除先清理文件再解除记录,中断后以 modified/missing 保守呈现而非误认领。
- **落盘适配**:Skills 完整目录复制(含附属文件与可执行权限),来源和目标均拒绝符号链接/Windows 重解析点与路径越界;每技能最大 64 MiB/10000 文件/32 层。临时目录写完后提升,保留旧目录以便失败回滚。MCP JSON 保留其他服务/设置,TOML 经 toml_edit 保留注释/排版;损坏配置禁止写入。Gemini 的 httpUrl/url、Codex 的 http_headers、OpenCode 的 local/remote/command 数组/environment 等在后端转换;不支持的传输类型禁选。资源库加密不会加密导出的 Agent 配置,面板必须提示明文凭据和 Git 提交风险。
- **Agent 显隐**:资源管理增加「Agent 工具」Tab(`ResourceAgentsTab.vue`),通过 settings.json 的 `hiddenResourceAgents: string[]` 保存本机偏好,缺省空数组(显示所有已适配目标),不随资源库备份同步,不要求本机安装。隐藏仅收起项目页按钮,已部署的隐藏目标仍可查看/移除;不得影响 Wiki 的 ACP 选择器。
- **预览**:`AiFileDrawer.vue` 新增 readOnly,项目 Skills/MCP(含非托管项)仅预览,内容统一去资源库维护;指令/规则文件仍可编辑,保存继续走 save_text_file。抽屉内部跳转也重新判断只读性,不能从指令链接绕回配置编辑。全局 AI 厂商页的 cc-switch 配置导入与本次项目资源改造无关,保持不变。

## AI 用量统计(commands/usage.rs + lib/ai-usage.ts)

- 每次 LLM 调用逐条写入 SQLite `ai_usage_log`(迁移 009):时间、任务类型(`commit / report / wiki / chat / translate / scan` 六类,日报周报并入 report,测试连接不计量;前后端取值一致)、模型、输入/输出/合计 tokens、耗时、缓存命中 tokens(cached_tokens,输入的子集:OpenAI 兼容的 prompt_tokens_details.cached_tokens / ACP 的 cachedReadTokens;迁移 010);token 列可空(provider 未返回 usage 时行仍在,计入调用次数但不计入 SUM)。展示在设置页「AI 用量」分类(`AiUsageSettings.vue`:汇总五格 + 最近半年热力图 + 任务类型分布列表(类型 · 次数 · 合计 tokens)+ 明细分页列表与清空;热力图网格/分档逻辑在 `lib/usage-heatmap.ts`,27 周 × 7 行周一起始,强度按窗口内最大日 tokens 分 4 档,有调用但无 tokens 的日子记最低档)。
- **三条采集链路**:①前端 commit/报告调用收敛在 `lib/ai.ts`(AI SDK 的 usage 已解析,generateText 解构 `usage`、streamText 消费 `result.usage`,SDK 自动带 `stream_options.include_usage`),经 `lib/ai-usage.ts` 的 `recordAiUsage` fire-and-forget 上报,**记录失败绝不影响生成主流程**;②Rust 侧:wiki 内置 Agent 与资源库技能安全扫描的语义层按 harness Usage 事件**逐次 LLM 请求**落库(task_type 分别为 "wiki"/"scan",含单请求耗时;多轮工具循环拆成多行,经共享的 `commands/ai/harness_support.rs` 采集),定时报告(scheduler.rs)`ChatResponse` 解析 `usage` 字段与报告历史同一事务落库(均复用 `insert_usage_row`);③ACP agent 后端:`agent-client-protocol` 开启 `unstable_end_turn_token_usage` feature 读 PromptResponse.usage——按**单次 prompt 口径**直接记录(不可对相邻响应做累计差分,否则后一请求用量较小时会误记为空);agent 未上报时用 `tiktoken-rs` 统计应用可见的 prompt 与最终正文(已知 OpenAI 模型选择对应编码器,未知/第三方模型回退 `o200k_base`),该兜底不含 agent 内部工具调用和上下文,属于保守估算,cached tokens 留空。
