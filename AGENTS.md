...
src-tauri/src/
  ...
  cli/                  内置 CLI(clap 子命令,JSON 输出;按域拆 *_tool.rs,技能文本在仓库根 skills/)
...
10. **内置 CLI + Skills**:`main.rs` 在 Tauri 初始化前检查首参数,命中 `wiki`/`sem`/`project`/`report`/`tag`/`script`/`pin`/`hidden`/`ai`/`prompt`/`account`/`file` 即进入 `src/cli/` 的 clap CLI(执行后退出,不开窗口);CLI 覆盖应用数据面管理能力(项目登记/归档/开关、标签、自定义命令、pin/hidden、Wiki 配置、报告历史与调度、AI 配置/用量、提示词、Git 账号、项目文件);git/docker 等本机工具不包装,agent 直接调用,实现复用 `commands/*` 的 `pub(crate)` 纯函数/`blocking` 变体;成功 JSON 写 stdout,失败 `{"code","message","detail"}` JSON 写 stderr 且退出码非 0。release 为 windows 子系统,CLI 模式先 `AttachConsole(ATTACH_PARENT_PROCESS)` 以便终端可见输出。配套内置技能 `repomeow`(仓库根 `skills/repomeow/`,SKILL.md 入口 + references/ 细分,通过 PATH 直接调用 `repomeow`,导入时不写入本机可执行路径)经「设置 → CLI」一键导入资源库,再由项目 AI 资源部署到各 agent;能力暴露由技能部署控制,无运行时开关。详见根目录 `CLI.md`。
