...
- chat 偏好热切换:`chat_send` 每次前置 busy 前重读配置,thinking/permission 变化就地生效且会话历史保留;模型与密钥由 `chat_stream_fn` 每次 LLM 调用重读解析(`catalog::resolve_model` 填全元数据,`Model::from_settings` 已门控为仅测试用)。工具权限两档均向模型暴露全部 12 个工具:`all` 直接执行;`ask` 通过 chat 专属 `before_tool_call` 硬门禁拦截 `update_wiki` / `regenerate_wiki` / `add_custom_command` / `generate_report`,向前端推 `ToolPermissionRequest` 并等待单次允许/拒绝,取消或 2 分钟超时自动阻断;旧配置值 `readOnly` 兼容归一为 `ask`。
