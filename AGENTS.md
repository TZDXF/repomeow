...
- **双生成后端**(`lib/wiki-generator.ts` 的 `WikiGenKernel` 接口,两种内核产出相同契约——大纲页列表 + 页 Markdown,落盘/状态机/UI 完全共用):①**内置 API**(默认,上节所述 OpenAI 兼容管线);②**本地 coding agent 后端**(经 ACP 协议调用户本机已登录的 agent CLI,复用订阅额度):精选 13 个 agent(claude/codex/gemini/copilot/grok/qwen/cline/glm/pi + 原生二进制 opencode/goose/cursor/kimi,条目 vendor 在 `commands/agent.rs`,数据源自 ACP registry;**对话框下拉只列已安装的项**,自定义命令行的后端支持保留但已无 UI 入口);agent 自行探索仓库(文件树/README 仅作提示),复用同一严格 JSON 大纲解析与 sources 注释块(行级引用为尽力而为),**单会话顺序生成**(并发固定 1),页面同样流式(`agent_message_chunk` 累积)。生成配置按项目独立保存为对应 wiki 目录的 `config.json`(`version` + `backend`,后端对象包含 agentId/model/thinking,空模型/思考强度 = agent 默认),不再写全局 settings;旧版全局 wiki agent 配置在项目首次读取时惰性复制为该项目 config,之后互不影响。wiki 页点「生成/重新生成」(或右上角配置入口,edit 模式确认=仅保存)弹出 `components/wiki/WikiGenerateDialog.vue`,表单只含后端/模型/思考强度三项(不展示安装路径、无「测试」按钮),打开时读取当前项目配置(配置指向未安装 agent 时归一回内置),选中 agent 后自动探测其模型/思考强度清单(`acpTestCached`,lib/agent.ts 的应用会话级 Promise 缓存,同一 agent 不重复 spawn),确认才写回当前项目,取消丢弃改动;整本生成、增量更新、单页重生成与自动更新均由 Rust 按项目路径读取同一配置。meta 记录 `generator` 字段("builtin"/"acp:<id>",serde default 向后兼容)与「agent 名称 · 所选模型」;自动更新使用项目配置对应的内置 API 或 agent(每次 agent 更新临时起会话)。
